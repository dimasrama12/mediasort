...
export function Sidebar() {
  ...
  return (
    <aside className="w-56 shrink-0 border-r border-[var(--border)] bg-[var(--panel)] flex flex-col overflow-auto">
      <div className="p-2 text-[11px] text-[var(--muted)] truncate" title={base}>
        {base || "No folder scanned"}
      </div>
      <ul className="flex-1">
        {folders.map((f) => (
          <li key={f.id} className="flex items-center gap-2 px-2 py-1 text-sm text-[var(--text)]">
            <kbd className="w-5 h-5 grid place-items-center rounded bg-[var(--elevated)] text-xs">{f.shortcut}</kbd>
            {editingId === f.id ? (
              <input
                autoFocus
                value={editName}
            ...
            <span className="text-[var(--muted)] text-xs">{f.fileCount}</span>
            <button
      ...
      {adding ? (
        ...
        <button
          ...
          disabled={full || !base}
          className="m-2 px-2 py-1 rounded bg-[var(--elevated)] hover:bg-[var(--elevated-hover)] disabled:opacity-40 text-sm"
        >
            ...
            {groups.map((g) => (
              <li key={g.id}>
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2 py-1 text-sm text-[var(--text)] hover:bg-[var(--elevated)] text-left"
                >
                  <span className="text-xs" aria-hidden>
                    {g.groupType === "visual" ? "◈" : "◷"}
                  </span>
                  <span className="flex-1 truncate">{g.name}</span>
                  <span className="text-[var(--muted)] text-xs">{g.fileIds.length}</span>
                  <span className="text-[var(--muted)] text-[10px] w-9 text-right">
